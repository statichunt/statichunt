...
export const getRegularPage = async (slug, url) => {
  const publishedPages = getSinglePage("content/regular-pages");
  ...
  // filter by ui
  const uiData = publishedThemesExamples.filter((theme) =>
    theme.frontmatter?.ui?.map((ui) => slugify(ui)).includes(slug),
  );
...
export const getRegularPageSlug = () => {
  const regularSlugs = getSinglePageSlug("content/regular-pages");
  ...
  const uiPage = getSinglePage("content/ui");
  const uiFiltering = uiPage.filter((page) => !page.frontmatter.page_draft);
  const uiThemesSlugs = uiFiltering.map((page) => page.slug);
