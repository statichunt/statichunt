...
export const getSinglePages = (folder, taxonomy) => {
...
export const getRegularPage = async (slug) => {
  ...
  // filter by archtype
  const archetypeData = publishedTheme.filter(
    (theme) =>
...
export const getSinglePagesSlug = (folder, isThemes) => {
  const publishedPages = getSinglePages(folder, isThemes);
...
export const getRegularPageSlug = () => {
  ...
  const slug = regularPage.map((page) => page.slug);
  const allSlug = [...slug, ...toolFile, ...ssgFile];
